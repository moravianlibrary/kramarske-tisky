if [ $# -ne 3 ] ; then
  echo "Usage: $0 input_file_tiff ouput_file_jp2k_archive_copy ouput_file_jp2k_user_copy"
else
  INPUT_FILE_TIFF=$1
  OUTPUT_FILE_JP2K_ARCHIVE_COPY=$2
  OUTPUT_FILE_JP2K_USER_COPY=$3
  ...
  if [ -f OUTPUT_FILE_JP2K_ARCHIVE_COPY ]; then
    echo "File OUTPUT_FILE_JP2K_ARCHIVE_COPY exists, deleting"
    rm OUTPUT_FILE_JP2K_ARCHIVE_COPY
  fi
  if [ -f OUTPUT_FILE_JP2K_USER_COPY ]; then
    echo "File OUTPUT_FILE_JP2K_USER_COPY exists, deleting"
    rm OUTPUT_FILE_JP2K_USER_COPY
  fi

  # tiff to jp2k with kakadu
  ...
  kdu_compress -i ${INPUT_FILE_TIFF} -o ${OUTPUT_FILE_JP2K_ARCHIVE_COPY} "Cblk={64,64}" Corder=RPCL "Stiles={4096,4096}" "Cprecincts={256,256},{256,256},{128,128}" ORGtparts=R Creversible=yes Clayers=1 Clevels=5 "Cmodes={BYPASS}" Cuse_sop=yes Cuse_eph=yes
  # master copy 1:8 quality
  #kdu_compress -i ${INPUT_FILE_TIFF} -o ${OUTPUT_FILE_JP2K_MASTER_COPY} "Cblk={64,64}" Corder=RPCL "Stiles={1024,1024}" "Cprecincts={256,256},{256,256},{128,128}" ORGtparts=R -rate 3 Clayers=12 Clevels=5 "Cmodes={BYPASS}"
  # master copy 1:10 quality
  kdu_compress -i ${INPUT_FILE_TIFF} -o ${OUTPUT_FILE_JP2K_USER_COPY} "Cblk={64,64}" Corder=RPCL "Stiles={1024,1024}" "Cprecincts={256,256},{256,256},{128,128}" ORGtparts=R -rate 2.4 Clayers=12 Clevels=5 "Cmodes={BYPASS}"
